function [ x, w, reccalls ] = gaussQuadratureHermite( nmax, tol, numpoint, reccalls, maxrec)
...
if tol(1) < 5*eps && reccalls ==0
    warning('this position tolerance is of order eps, function may fail')
end
if length(tol)==1
    tol = [tol,1];%no condition on tol 2
end
if tol(2) < nmax*eps && reccalls ==0
    warning('second convergence condition unlikely to be achieved')
end
if nargin < 3
...
if sum(underflowchk)~=0
    warning('some x values cut off due to underflow of psi_0(x), may still be fine or may cut off zeros')
...
if zcount < ceil(nmax/2)
    warning('not enough zeros found use more points or nmax too large')
    zcount 
    w=nan;
    return
end
if zcount > ceil(nmax/2)
    warning('failed to distinguish zeros for some reason')
    zcount 
    w=nan;
    return
end
...
if reccalls < maxrec
   ...
   [ x,w,reccalls ] = util_yb.gauss_hermite( nmax,tol,finex,reccalls,maxrec ); 
   return
else
